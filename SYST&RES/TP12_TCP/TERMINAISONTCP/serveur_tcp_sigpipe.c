...
int main(int argc, char *argv[]) {
  ...
  while (TRUE) {
    ...
    ************************************************************/
	...
	/* b) On ecrit en deux coups */
	if((rep = send(socket_service, &(ch[0]), 1, 0))==-1){
	  perror("send");
	  //exit(2);
	}
	else
	  printf("server sent (%d) : %c \n", 1, ch[0]);

	/* Le deuxieme send genere un SIGPIPE */
	if(send(socket_service, &(ch[1]), rep-1, 0)==-1){
	  perror("send");
	  //exit(2);
	}
	else
	...
	/*If you are using buffered TCPIP, then a successful write just
	means your data has been queued to send, it doesn't mean it
	has been sent. Until you successfully call close, you don't
